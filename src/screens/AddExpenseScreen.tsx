...
export const AddExpenseScreen: React.FC<AddExpenseScreenProps> = ({
  ...
  const handleSave = async () => {
    // Use selected category name as title if title is empty
    const finalTitle = title.trim() || category || 'مصروف';
    
    if (!finalTitle) {
      alertService.warning('تنبيه', 'يرجى إدخال عنوان المصروف أو اختيار الفئة');
      return;
    }
    ...
    try {
      ...
      if (expense) {
        ...
        alertService.success('نجح', 'تم تحديث المصروف بنجاح');
      } else {
  ...
  const handleCancelEditShortcut = () => {
    if (editingShortcut) {
      // Restore form to original state if editing expense, otherwise reset
      if (expense) {
        setTitle(expense.title);
        setAmount(expense.amount.toString());
        setCategory(expense.category as ExpenseCategory);
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        // Auto-fill title field with category name if empty
                        if (!title.trim()) {
                          setTitle(cat.name);
                        }
                      }}
...
// Reuse styles from AddIncomeScreen - they're identical
