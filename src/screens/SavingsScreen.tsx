...
import { getSavings, deleteSavings, addSavingsTransaction, transferBetweenSavings } from '../database/database';
...
export const SavingsScreen = ({
  ...
  const handleTransactionConfirm = async (amount: number, type: 'deposit' | 'withdrawal') => {
    ...
    try {
      ...
      loadSavings();
      alertService.toastSuccess(type === 'deposit' ? tl("تمت إضافة المبلغ بنجاح") : tl("تم سحب المبلغ بنجاح"));
    } catch (error) {
      alertService.toastError(tl("حدث خطأ أثناء العملية"));
      throw error;
    }
  };
  const handleTransferConfirm = async (fromId: number, toId: number, amount: number) => {
    try {
      await transferBetweenSavings(fromId, toId, amount, new Date().toISOString().split('T')[0]);
      loadSavings();
      alertService.toastSuccess(tl("تم تحويل المبلغ بنجاح"));
    } catch (error) {
      alertService.toastError(tl("حدث خطأ أثناء عملية التحويل"));
      throw error;
    }
  };
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={theme.colors.primary} />} showsVerticalScrollIndicator={false}>
        ...
        y: 1
          ...
          <View style={styles.summaryFooter}>
            <Text style={styles.summaryFooterText}>{tl("لديك")}{savingsList.length}{tl("حصالات نشطة")}</Text>
          </View>
        </LinearGradient>

        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>{tl("حصالاتي")}</Text>
          <TouchableOpacity onPress={handleAddSavings} style={styles.addSmallButton}>
            <Ionicons name="add" size={20} color={theme.colors.primary} />
            <Text style={styles.addSmallButtonText}>{tl("جديدة")}</Text>
          </TouchableOpacity>
        </View>

        {savingsList.length > 0 ? savingsList.map(item => <SavingsCard key={item.id} savings={item} onPress={handleSavingsPress} onAddAmount={handleAddAmount} onEdit={s => navigation.navigate('AddSavings', {
        savings: s
      })} onDelete={async s => {
        try {
          await deleteSavings(s.id);
          loadSavings();
          alertService.toastSuccess(tl("تم حذف الحصالة بنجاح"));
        } catch (error) {
          alertService.toastError(tl("حدث خطأ أثناء حذف الحصالة"));
        }
      }} onTransfer={handleTransferPress} />) : <View style={styles.emptyState}>
            <View style={styles.emptyIconContainer}>
              <Ionicons name="wallet-outline" size={64} color={theme.colors.textMuted} />
            </View>
            <Text style={styles.emptyTitle}>{tl("لا توجد حصالات بعد")}</Text>
            <Text style={styles.emptySubtitle}>{tl("ابدأ بإنشاء حصالة جديدة لتوفير المال لأهدافك المختلفة")}</Text>
            <TouchableOpacity style={styles.createButton} onPress={handleAddSavings}>
              <Text style={styles.createButtonText}>{tl("إنشاء حصالة الآن")}</Text>
            </TouchableOpacity>
          </View>}
      </ScrollView>


      <AddSavingsTransactionModal visible={showTransactionModal} savings={selectedSavings} onClose={() => setShowTransactionModal(false)} onConfirm={handleTransactionConfirm} />

      <SavingsTransferModal visible={showTransferModal} fromSavings={selectedSavings} allSavings={savingsList} onClose={() => setShowTransferModal(false)} onTransfer={handleTransferConfirm} />
    </ScreenContainer>;
};
