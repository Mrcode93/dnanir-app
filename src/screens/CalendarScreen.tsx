    ...
    const fetchMonthActivity = useCallback(async () => {
        try {
            ...
            const [expenses, income] = await Promise.all([
                getExpensesPaginated({ startDate: start, endDate: end }),
                getIncomePaginated({ startDate: start, endDate: end })
            ]);
    ...
    const fetchTransactions = useCallback(async () => {
        ...
        try {
            ...
            const [expenses, income, categories] = await Promise.all([
                getExpensesPaginated({ startDate: dateStr, endDate: dateStr }),
                getIncomePaginated({ startDate: dateStr, endDate: dateStr }),
                getCustomCategories()
            ]);

            // Tag transactions with their type
            const taggedExpenses = expenses.map(e => ({ ...e, type: 'expense' as const }));
            const taggedIncome = income.map(i => ({ ...i, type: 'income' as const }));
                ...
                {calendarGrid.map((day, index) => {
                    ...
                    return (
                        <TouchableOpacity
                            ...
                            style={[
                                styles.dayItemOuter,
                                !isCurrentMonth && styles.notCurrentMonthItem
                            ]}
                        ...
                        >
                            {isSelected ? (
                                ...
                                <View style={[
                                    styles.dayItemInner,
                                    !isDayToday && monthActivity.has(formatDateLocal(day)) && styles.activeDayInner
                                ]}>
                                    <Text style={[
                                        styles.dayText,
    ...
    return (
        <ScreenContainer scrollable={false}>
                    ...
                    <View style={styles.transactionsSection}>
                        <View style={styles.sectionHeader}>
                            <View style={styles.sectionTitleRow}>
                                <Text style={styles.sectionTitle}>عمليات يوم {selectedDate.getDate()} {MONTH_NAMES[selectedDate.getMonth()]}</Text>
                                <View style={styles.transactionCountBadge}>
                                    <Text style={styles.transactionCountText}>{transactions.length}</Text>
                                </View>
                            </View>
                            <TouchableOpacity
                                onPress={() => navigation.navigate('AddExpense', { initialDate: selectedDate })}
                ...
                <View style={styles.listContainer}>
                    {loading ? (
                        ...
                        transactions.map((item) => (
                            <View key={`${(item as any).type}-${item.id}`} style={styles.itemWrapper}>
                                <TransactionItem
                                    item={item}
                        ...
                        <View style={styles.emptyContainer}>
                            ...
                            <Text style={styles.emptyText}>لا توجد أي عمليات مسجلة في هذا التاريخ</Text>
                            <TouchableOpacity
                                style={styles.emptyAddBtn}
                                onPress={() => navigation.navigate('AddExpense', { initialDate: selectedDate })}
                            >
                                <Text style={styles.emptyAddBtnText}>أضف عملية الآن</Text>
                            </TouchableOpacity>
                        </View>
                    )}
            ...
            <TransactionDetailsModal
                ...
                customCategories={customCategories}
                onClose={() => { setShowDetails(false); setSelectedTransaction(null); }}
                onEdit={() => {
