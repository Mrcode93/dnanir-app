...
export const GoalPlanScreen = ({
  ...
  const fetchPlan = useCallback(async () => {
    ...
    try {
      ...
      const [currentMonthData, previousMonthData] = await Promise.all([getCurrentMonthData(), getMonthData(prevYear, prevMonth)]);
      ...
      if (result.success && result.data) {
        ...
        setError(result.error || tl("فشل في إنشاء الخطة"));
  ...
  return <SafeAreaView style={styles.container} edges={['bottom']}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          ...
          }]} numberOfLines={1}>{goal.title}</Text>
            <Text style={[styles.headerSubtitle, {
            color: theme.colors.textSecondary
          }]}>
              {isPrivacyEnabled ? `**** / **** ${goalCurrency}` : `${goal.currentAmount.toLocaleString('ar-IQ-u-nu-latn')} / ${goal.targetAmount.toLocaleString('ar-IQ-u-nu-latn')} ${goalCurrency}`}
            </Text>
          </View>
          <TouchableOpacity style={styles.refreshBadge} onPress={fetchPlan} disabled={loading}>
            <LinearGradient colors={primaryGradient} start={{
            x: 0,
            y: 0
          }} end={{
            x: 1,
        ...
        {!plan && !loading && <View style={[styles.emptyState, {
        ...
        borderColor: theme.colors.border
          ...
          color: theme.colors.textPrimary
        }]}>{tl("لا توجد خطة حالياً")}</Text>
            <Text style={[styles.emptyStateText, {
          color: theme.colors.textSecondary
        }]}>{tl("اضغط الزر أعلاه للحصول على خطة ونصائح لتحقيق هذا الهدف بالذكاء الاصطناعي.")}</Text>
            ...
            y: 1
                ...
                <Text style={styles.largeAnalyzeBtnText}>{tl("إنشاء الخطة الآن")}</Text>
              </LinearGradient>
            </TouchableOpacity>
          </View>}

        {plan && <>
            {cachedAt != null && <Text style={[styles.cachedLabel, {
          color: theme.colors.textMuted
        }]}>{tl("آخر تحديث:")}{new Date(cachedAt).toLocaleDateString(language === 'ar' ? 'ar-IQ-u-nu-latn' : 'en-US')}
              </Text>}

            {plan.message ? <LinearGradient colors={isDark ? ['#1E293B', '#0F172A'] : ['#FFFFFF', '#F8F9FA']} style={[styles.statusCard, {
          borderColor: theme.colors.primary + '40'
            ...
            {plan.suggestedMonthlySaving != null && plan.suggestedMonthlySaving > 0 && <View style={styles.section}>
              ...
              color: theme.colors.textPrimary
            }]}>{tl("الادخار الشهري المقترح")}</Text>
                </View>
            ...
            {plan.planSteps.length > 0 && <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  <View style={[styles.sectionIcon, {
              backgroundColor: theme.colors.primary + '20'
            }]}>
              ...
              color: theme.colors.textPrimary
                ...
                {plan.planSteps.map((step, i) => <View key={i} style={[styles.actionItemCard, {
            backgroundColor: theme.colors.surfaceCard
          }]}>
                    <LinearGradient colors={primaryGradient} style={styles.priorityBadge}>
            ...
            {plan.tips.length > 0 && <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  <View style={[styles.sectionIcon, {
              ...
              color: theme.colors.textPrimary
                ...
                {plan.tips.map((tip, i) => <View key={i} style={[styles.tipCard, {
            ...
            y: 1
                ...
                <Text style={styles.bottomAnalyzeBtnText}>{tl("إعادة إنشاء الخطة")}</Text>
              </LinearGradient>
            </TouchableOpacity>
          </>}
      </ScrollView>
