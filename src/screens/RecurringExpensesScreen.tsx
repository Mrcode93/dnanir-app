...
import { View, StyleSheet, FlatList, RefreshControl, Text, TouchableOpacity, Platform } from 'react-native';
...
export const RecurringExpensesScreen = ({
  ...
  const getCategoryName = (category: string) => {
    return tl(EXPENSE_CATEGORIES[category as keyof typeof EXPENSE_CATEGORIES] || customCategories.find(c => c.name === category)?.name || category);
  };
  const getRecurrenceText = (expense: RecurringExpense) => {
    ...
    if (expense.recurrenceValue === 1) {
      return tl("كل {{}}", [types[expense.recurrenceType]]);
    }
    return tl("كل {{}} {{}}", [expense.recurrenceValue, types[expense.recurrenceType]]);
  };
  const renderRecurringExpense = useCallback(({
    item
  }: {
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.cardDetails}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailText}>{tl("التالي:")}{nextDate}</Text>
            </View>
            {!item.isActive && <View style={styles.inactiveBadge}>
                <Text style={styles.inactiveText}>{tl("غير نشط")}</Text>
              </View>}
          </View>
  ...
  return <ScreenContainer edges={['top', 'bottom']}>
      {recurringExpenses.length === 0 ? <View style={styles.emptyContainer}>
          <Ionicons name="repeat-outline" size={80} color={theme.colors.textSecondary} />
          <Text style={styles.emptyText}>{tl("لا توجد مصاريف متكررة")}</Text>
          <Text style={styles.emptySubtext}>{tl("أضف مصروف متكرر لتتبعه تلقائياً")}</Text>
        </View> : <FlatList data={recurringExpenses} renderItem={renderRecurringExpense} keyExtractor={item => item.id.toString()} contentContainerStyle={styles.listContent} initialNumToRender={10} maxToRenderPerBatch={8} windowSize={7} updateCellsBatchingPeriod={50} removeClippedSubviews={Platform.OS === 'android'} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} />}
      ...
      setShowDeleteAlert(false);
      setExpenseToDelete(null);
    }} onConfirm={confirmDelete} title={tl("حذف المصروف المتكرر")} message={tl("هل أنت متأكد من حذف هذا المصروف المتكرر؟")} confirmText={tl("حذف")} cancelText={tl("إلغاء")} type="danger" />
    </ScreenContainer>;
};
